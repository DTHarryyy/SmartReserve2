...
begin
  ...
  with facility_scope as (
    ...
    select p.*, r.total_amount_centavos, r.reservation_status
    from public.payment_transactions p
    join public.reservation_requests r on r.id = p.request_id
    join facility_scope f on f.id = r.facility_id
    where r.admin_lane = lane_value
  ), revenue as (
    select
      coalesce(sum(amount_centavos) filter (
        where status = 'verified'
          and purpose <> 'refund'
          and verified_at >= p_from
          and verified_at < p_to
      ), 0)::integer as gross_verified_centavos,
      coalesce(sum(amount_centavos) filter (
        where status = 'refunded'
          and coalesce(refunded_at, verified_at, updated_at) >= p_from
          and coalesce(refunded_at, verified_at, updated_at) < p_to
      ), 0)::integer as refunds_centavos,
      count(*) filter (
        where status = 'verified'
          and purpose <> 'refund'
          and verified_at >= p_from
          and verified_at < p_to
      )::integer as verified_payment_count,
      count(distinct request_id) filter (
        where status = 'verified'
          and purpose <> 'refund'
          and verified_at >= p_from
          and verified_at < p_to
      )::integer as paid_reservation_count
    from scoped_payments
  ), outstanding as (
    select coalesce(sum(greatest(
      0,
      r.total_amount_centavos - coalesce(verified.verified_centavos, 0)
    )), 0)::integer as outstanding_centavos
    from public.reservation_requests r
    join facility_scope f on f.id = r.facility_id
    join public.reservation_occurrences o on o.request_id = r.id
    left join lateral (
      select coalesce(sum(p.amount_centavos), 0)::integer as verified_centavos
      from public.payment_transactions p
    ...
    where r.admin_lane = lane_value
      and r.reservation_status in ('awaiting_payment', 'confirmed', 'completed')
      and o.starts_at < p_to
      and o.ends_at > p_from
  ), month_bounds as (
    select generate_series(
      date_trunc('month', p_from at time zone 'Asia/Manila')::date,
      date_trunc('month', (p_to - interval '1 microsecond') at time zone 'Asia/Manila')::date,
      interval '1 month'
    )::date as month_start
  ), monthly as (
    select coalesce(jsonb_agg(jsonb_build_object(
      'month_start', (m.month_start::timestamp at time zone 'Asia/Manila'),
      'submitted_reservations', coalesce(res.submitted_reservations, 0),
      'confirmed_reservations', coalesce(res.confirmed_reservations, 0),
      'completed_occurrences', coalesce(occ.completed_occurrences, 0),
      'booked_hours', round(coalesce(occ.booked_hours, 0)::numeric, 2),
      'utilisation_fraction', case when coalesce(hrs.available_hours, 0) = 0 then 0
        else least(1::numeric, coalesce(occ.booked_hours, 0)::numeric / hrs.available_hours) end,
      'gross_verified_centavos', coalesce(pay.gross_verified_centavos, 0),
      'refunds_centavos', coalesce(pay.refunds_centavos, 0),
      'net_revenue_centavos', coalesce(pay.gross_verified_centavos, 0) - coalesce(pay.refunds_centavos, 0)
    ) order by m.month_start), '[]'::jsonb) as value
    from month_bounds m
    left join lateral (
      select
        count(*)::integer as submitted_reservations,
        count(*) filter(where r.reservation_status in ('confirmed', 'completed'))::integer as confirmed_reservations
      from public.reservation_requests r
      join facility_scope f on f.id = r.facility_id
      where r.admin_lane = lane_value
        and (r.created_at at time zone 'Asia/Manila')::date >= m.month_start
        and (r.created_at at time zone 'Asia/Manila')::date < (m.month_start + interval '1 month')::date
    ) res on true
    left join lateral (
      select
        count(*) filter(where o.lifecycle_stage = 'completed')::integer as completed_occurrences,
        coalesce(sum(extract(epoch from (o.ends_at - o.starts_at)) / 3600) filter(where o.booking_state = 'booked'), 0) as booked_hours
      from public.reservation_occurrences o
      join public.reservation_requests r on r.id = o.request_id
      join facility_scope f on f.id = o.facility_id
      where r.admin_lane = lane_value
        and (o.starts_at at time zone 'Asia/Manila')::date >= m.month_start
        and (o.starts_at at time zone 'Asia/Manila')::date < (m.month_start + interval '1 month')::date
    ) occ on true
    left join lateral (
      select coalesce(sum(greatest(0, extract(epoch from(
        least(((d::date + f.close_time) at time zone 'Asia/Manila'), ((m.month_start + interval '1 month')::timestamp at time zone 'Asia/Manila')) -
        greatest(((d::date + f.open_time) at time zone 'Asia/Manila'), (m.month_start::timestamp at time zone 'Asia/Manila'))
      )) / 3600)) filter (
        where f.open_days[extract(isodow from d)::integer]
      ), 0)::numeric as available_hours
      from facility_scope f
      cross join generate_series(
        m.month_start,
    ...
    left join lateral (
      select
        coalesce(sum(amount_centavos) filter(where status = 'verified' and purpose <> 'refund'), 0)::integer as gross_verified_centavos,
        coalesce(sum(amount_centavos) filter(where status = 'refunded'), 0)::integer as refunds_centavos
      from scoped_payments p
      where (coalesce(
          case when p.status = 'refunded' then coalesce(p.refunded_at, p.verified_at, p.updated_at) end,
          p.verified_at,
          p.updated_at
        ) at time zone 'Asia/Manila')::date >= m.month_start
        and (coalesce(
          case when p.status = 'refunded' then coalesce(p.refunded_at, p.verified_at, p.updated_at) end,
          p.verified_at,
          p.updated_at
        ) at time zone 'Asia/Manila')::date < (m.month_start + interval '1 month')::date
    ) pay on true
  )
  select jsonb_build_object(
    ...
    'revenue', jsonb_build_object(
      'gross_verified_centavos', revenue.gross_verified_centavos,
      'refunds_centavos', revenue.refunds_centavos,
      'net_revenue_centavos', revenue.gross_verified_centavos - revenue.refunds_centavos,
      'outstanding_centavos', (select outstanding_centavos from outstanding),
      'verified_payment_count', revenue.verified_payment_count,
      'paid_reservation_count', revenue.paid_reservation_count
    ),
    'monthly_statistics', (select value from monthly)
  ) into report_value
  from summary s
  cross join revenue;

  -- Revenue is restricted to external administrators: null it out for
  -- internal-admin callers so a direct RPC call can't bypass the client gate.
  if not public.is_external_admin() then
    report_value := jsonb_set(
      report_value,
      '{revenue}',
      jsonb_build_object(
    ...
    report_value := jsonb_set(
      ...
      coalesce((
        select jsonb_agg(
          elem - 'gross_verified_centavos' - 'refunds_centavos' - 'net_revenue_centavos'
        )
        from jsonb_array_elements(report_value -> 'monthly_statistics') elem
      ), '[]'::jsonb)
    );
  end if;
