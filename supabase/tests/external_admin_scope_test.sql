...
select '95000000-0000-0000-0000-000000000001',user_id,
  ...
  'Computer Laboratory 1','CICS',40,'Paid scope test',10,'approved',false,50000,'authorized','2030-01-01'
from actors
union all
select '95000000-0000-0000-0000-000000000002',user_id,
  '10000000-0000-0000-0000-000000000001','Verified Client','user',
  'Computer Laboratory 1','CICS',40,'Free scope test',10,'approved',false,0,'not_required','2030-01-01'
from actors;
...
select ok(public.can_access_reservation('95000000-0000-0000-0000-000000000001'),
  'external admins can access a released paid reservation');
select isnt(public.can_access_reservation('95000000-0000-0000-0000-000000000002'),true,
  'external admins cannot access a free reservation');
...
select is(
  ...
  1::numeric,'external reports count paid bookings only');
